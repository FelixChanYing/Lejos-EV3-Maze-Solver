...
public class ActualRun extends Thread
{
	...
	g.setFont(Font.getSmallFont());
        ...
        LCD.drawString("Please set amplifier", 0, 5);
        do
        {
	        if (Button.DOWN.isUp())
	        {
	        	amplifier=amplifier-5;
	        	LCD.drawString("Amplifier: "+amplifier, 2, 6);
	        }
	        if (Button.UP.isUp())
	        {
	        	amplifier=amplifier+5;
...
//	boolean turn=false;
	...
	void PID()
	{
		do
		{
			...
			if (turnangle<0)//on white turning right
			{
				...
				rightmotor.forward();
				leftmotor.setSpeed((int) (speed+4*turnangle*turnangle)/2);
				leftmotor.forward();
				whitecounter++;	
	...
	{
		ActualRun follow = new ActualRun();
		follow.setup();
		follow.PID();
	}
}
